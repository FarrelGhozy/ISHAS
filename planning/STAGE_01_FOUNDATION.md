...
**Tujuan:** menyiapkan tanah tempat seluruh berdiri: aplikasi ISHAS baru, model data baru,
login tiga peran, dan `/` merender dashboard publik.

## Prasyarat

- Baca `docs/AGENTS.md`, `README.md`, `DATA_MODEL.md`; pola struktur meniru
 `~/Documents/02_Projek/HIBAH_INTERNAL` (app/ + features/ + shared/ + mocks/).
- lama (aplikasi lama) TIDAK disentuh sama sekali; tidak ada arsip/pemindahan dari lama.
...
- [x] `mocks/store/mock-store.ts`: action kirim/terima/tolak/ubah-status/hapus versi stub
 yang konsisten (implementasi penuh di Stage 03/05/06). Tindakan yang belum memenuhi aturan
...
- [x] Tombol reset di Pengaturan Admin mengembalikan seed.
...
- [x] `/` merender dashboard publik sejak awal (versi sederhana boleh: header publik + pemilih +
 agregat dari selector baru; penyempurnaan visual di Stage 02); tanpa landing, tanpa redirect.
