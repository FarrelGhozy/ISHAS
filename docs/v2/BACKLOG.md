...
- D-01 dijawab: **aplikasi terpisah bernama ISHAS** — React Router, file dipecah per folder fitur,
  dijalankan dengan bun 1.4; V1 tidak diubah; referensi pola: `~/Documents/02_Projek/HIBAH_INTERNAL`.
  Lokasi folder aplikasi baru menunggu konfirmasi pemilik.
...
## Tahap A (V2-01) — Fondasi (dependensi: tidak ada)
...
- A3. `/` merender dashboard publik dari awal (tanpa landing, tanpa redirect); landing tidak dimigrasi dari V1.
  Selesai bila: `/` tanpa login menampilkan dashboard, tanpa redirect.

## Tahap B (V2-02–V2-04) — Publik (dependensi: A)
...
## Tahap C (V2-05–V2-07) — Moderasi (dependensi: B)
...
## Tahap D (V2-08) — Self-assessment (dependensi: A, B2, C1)
...
## Tahap E (V2-09) — Admin & sinkron dokumen (dependensi: C)

- E1. Admin: verifikasi pesantren, buat akun pengelola → pemilih publik bertambah; hapus opsi asesor.
- E2. D-01 dijawab: aplikasi terpisah — tidak ada sinkronisasi dokumen root/FRONTEND_RULES V1. Yang diperlukan adalah dokumen proyek ISHAS baru (README, flow) yang ditulis bersama pembangunan; V1 dibiarkan utuh.
- E3. Pemeriksaan akhir: 27 pola route kanonis + kasus redirect/URL lama, guard anonim/3 peran, desktop/tablet/ponsel, lint, TypeScript, test, build → Stage 12 `REVIEW`.
