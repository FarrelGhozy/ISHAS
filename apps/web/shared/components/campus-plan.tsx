...
function usePlanImage(assetId: string) {
  ...
  useEffect(() => {
    if (assetId.startsWith("/images/")) {
      setResult({ id: assetId, url: assetId });
      return;
    }
    let disposed = false,
      objectUrl = "";
    getCampusAsset(assetId)
      .then((blob) => {
        if (disposed) return;
        if (!blob) {
          setResult({ id: assetId, error: "Gambar denah tidak tersedia pada perangkat ini." });
          return;
        }
      ...
      .catch(() => {
        if (!disposed) setResult({ id: assetId, error: "Gambar denah gagal dimuat." });
      });
    return () => {
      disposed = true;
  ...
  return {
    ...(result.id === assetId ? result : { id: assetId }),
    retry: () => {
      setResult({ id: "" });
...
export function CampusPlan({
  ...
  if (error || broken)
    return (
      <div role="status" className="rounded-lg bg-strip p-4 text-sm text-secondary-text">
        <AlertTriangle size={18} className="mb-2" />
        {error ?? "Gambar denah gagal dimuat."}
        <button
          type="button"
  ...
  if (!url)
    return (
      <div
        role="status"
        className="grid min-h-48 place-items-center rounded-lg bg-strip text-sm text-secondary-text"
      >
        Memuat denah…
      </div>
    );
  return (
    <div
      className="relative w-full overflow-hidden rounded-lg border border-line bg-strip"
      style={{ aspectRatio: `${plan.width} / ${plan.height}` }}
    >
      <img
        src={url}
        alt={`Denah gambaran besar pesantren · versi ${plan.revision}`}
        draggable={false}
      ...
      {onPoint ? (
        <button
          type="button"
          className="absolute inset-0 cursor-crosshair focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
          aria-label="Pilih titik pada denah dengan klik atau ketuk. Untuk keyboard gunakan kolom koordinat di bawah."
          onClick={(event) => {
            ...
            if (isValidPoint(point))
              onPoint({ x: Math.round(point.x * 100) / 100, y: Math.round(point.y * 100) / 100 });
          }}
        />
      ) : null}
      {children}
...
export function LocationPicker({
  ...
  onChange: (value: LocationSnapshot) => void;
  disabled?: boolean;
}) {
  ...
  const update = (next: PlanPoint | null, floorNote = value?.floorNote ?? "") =>
    onChange({
      areaId: value?.areaId,
      locationText: value?.locationText ?? "",
      floorNote,
      campusPlanVersionId: next ? (plan?.id ?? null) : null,
      point: next,
    });
  return (
    <section
      className="space-y-3 rounded-lg border border-line p-3"
      aria-label="Titik lokasi pelaporan"
    >
      <div>
        <h3 className="text-sm font-bold text-heading">
          Titik pada denah <span className="font-normal text-secondary-text">(opsional)</span>
        </h3>
        <p className="mt-1 text-sm text-secondary-text">
          Tandai posisi pada gambaran besar pesantren. Lantai cukup ditulis sebagai keterangan.
        </p>
      </div>
      {stale ? (
      ...
      {plan ? (
        <>
          <CampusPlan
            key={plan.id}
            plan={plan}
            onPoint={disabled ? undefined : (next) => update(next)}
          >
            {point ? (
              <span
                aria-hidden
          ...
          <details>
            <summary className="cursor-pointer text-sm font-semibold text-primary">
              Pilih titik dengan keyboard / koordinat
            </summary>
            <div className="mt-2 grid grid-cols-2 gap-3">
              {(["x", "y"] as const).map((axis) => (
                <label key={axis} className="text-sm">
                  {axis === "x" ? "Posisi horizontal (%)" : "Posisi vertikal (%)"}
                  <input
                    type="number"
                    min={0}
                    max={100}
        ...
        <p role="status" className="rounded bg-strip p-3 text-sm text-secondary-text">
          Denah pesantren belum tersedia. Tetap isi area atau keterangan lokasi.
        </p>
      )}
      {value?.point ? (
        <div className="flex flex-wrap items-center gap-3 text-sm">
          <span>
            {point
              ? `Titik dipilih: ${point.x}% · ${point.y}%`
              : "Titik masih pada denah sebelumnya"}
          </span>
          <button
            type="button"
        ...
        <p className="text-sm text-secondary-text">
          Belum ada titik. Lokasi area/teks tetap wajib; titik baru tampil publik setelah validasi.
        </p>
      )}
        ...
        <input
          ...
          onChange={(event) =>
            onChange({
              areaId: value?.areaId,
              locationText: value?.locationText ?? "",
              floorNote: event.target.value,
              campusPlanVersionId: value?.campusPlanVersionId ?? null,
