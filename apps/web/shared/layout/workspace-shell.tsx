...
export default function WorkspaceLayout() {
  ...
  const navigation = (
    <nav aria-label="Menu ruang kerja" className="space-y-1 p-3">
      ...
      <Link to="/" className="secondary-button mt-4 w-full">
        Dashboard publik
      </Link>
    </nav>
  );
  ...
  return (
    <div className="flex min-h-dvh">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex min-h-[68px] items-center gap-2 border-b border-line bg-white px-3 py-2 sm:px-4">
          ...
          <div className="ms-auto flex min-w-0 items-stretch gap-2">
            <button
              ...
              aria-haspopup="dialog"
              aria-label={`Notifikasi (${notifications.filter((n) => !n.read).length} belum dibaca)`}
            >
              <Bell size={18} />
            ...
            <div ref={profileRef} className="relative min-w-0">
              <button
                type="button"
                className="flex min-h-11 min-w-0 items-center gap-2 rounded-[7px] border border-line-soft bg-white px-2 text-left hover:bg-strip sm:min-w-56"
                aria-haspopup="menu"
              ...
              >
                ...
                <span className="hidden min-w-0 flex-1 text-xs leading-snug sm:block">
                  <strong className="block truncate text-heading">{user.name}</strong>
                  <span className="block truncate text-secondary-text">{user.role}</span>
                </span>
                <ChevronDown
                  size={15}
                  className={`hidden shrink-0 text-faint transition-transform sm:block ${profileOpen ? "rotate-180" : ""}`}
                  aria-hidden
                />
              </button>
              {profileOpen ? (
                ...
                >
                  <div className="flex items-center gap-3 p-4">
                    ...
                    <span className="min-w-0 text-xs leading-relaxed">
                      <strong className="block truncate text-heading">{user.name}</strong>
                      <span className="block truncate text-secondary-text">{user.email}</span>
                    </span>
                  </div>
                  <div className="flex items-center gap-2 border-y border-line bg-strip px-4 py-3 text-xs text-secondary-text">
                  ...
                  <button
                    ...
                    onClick={() => {
                      try {
                        ...
                        setError(
                          "Sesi tidak dapat dihapus. Periksa izin penyimpanan browser lalu coba lagi.",
                        );
                      }
                    }}
                  >
        ...
        {error ? (
          <p role="alert" className="p-4 text-sm text-[#b91c1c]">
            {error}
          </p>
        ) : null}
            ...
            {notifications.map((n) => (
              <li key={n.id}>
                <Link
                  to={n.targetUrl}
                  onClick={() => setNotifOpen(false)}
                  className="block rounded-lg px-2 py-3 text-sm hover:bg-strip"
                >
                  <span className="block font-semibold text-heading">{n.message}</span>
                  <span className="text-xs text-secondary-text">
                    {new Date(n.at).toLocaleString("id-ID")}
                  </span>
                </Link>
              </li>
            ))}
