...
export function validateMapLocation(
  ...
  if (
    ...
    typeof location.floorNote !== "string" ||
    location.locationText.length > 200 ||
    location.floorNote.length > 80
  )
    return "Keterangan lokasi tidak sah.";
  if (
    location.areaId &&
    !state.areas.some((area) => area.id === location.areaId && area.institutionCode === code)
  )
    return "Area titik tidak sah untuk pesantren ini.";
  ...
  const plan = state.campusPlans.find(
    (item) => item.id === location.campusPlanVersionId && item.institutionCode === code,
  );
  if (!plan || institution?.activeCampusPlanVersionId !== plan.id)
    return "Denah telah berubah. Pilih ulang titik pada denah terbaru atau hapus titik.";
  return null;
}
...
export function snapshotLocation(
  ...
  return {
    areaId: area?.id,
    locationText:
      area?.name ?? manualLocation?.trim() ?? map?.locationText ?? "Lokasi belum tersedia",
...
export function selectPublicCampusMap(state: IshasState, code?: string) {
  if (!code || !selectRegisteredInstitutions(state).some((item) => item.code === code))
    return { plans: [], activeId: undefined, items: [] as PublicMapItem[] };
  const plans = state.campusPlans
    .filter((plan) => plan.institutionCode === code)
    .sort((a, b) => b.revision - a.revision);
  const reports = selectPublicReports(state, code).filter(
    (report) => report.handlingStatus !== "Completed",
  );
  ...
  const items: PublicMapItem[] = state.findings
    ...
    .map((finding, index) => {
      ...
      const valid =
        location?.point &&
        isValidPoint(location.point) &&
        plans.some((plan) => plan.id === location.campusPlanVersionId);
      return {
        key: `temuan-${index + 1}`,
        issue: finding.issue,
        location: location?.locationText ?? finding.location,
        floor: location?.floorNote ?? finding.floor,
        level: finding.level,
        status: report.handlingStatus,
        point: valid ? { ...location!.point! } : null,
        versionId: valid ? location!.campusPlanVersionId : null,
        validator:
          report.validatedByName ??
          state.users.find((user) => user.id === report.validatedBy)?.name ??
          "",
  ...
  return {
    plans,
    activeId: state.institutions.find((item) => item.code === code)?.activeCampusPlanVersionId,
    items,
  };
}
...
export function clusterMapItems(items: PublicMapItem[], width = 1200, height = 800) {
  ...
  for (const item of items.filter((entry) => entry.point)) {
    const group = groups.find(
      (entries) =>
        Math.hypot(
          ((entries[0].point!.x - item.point!.x) * width) / 100,
          ((entries[0].point!.y - item.point!.y) * height) / 100,
        ) < 48,
    );
