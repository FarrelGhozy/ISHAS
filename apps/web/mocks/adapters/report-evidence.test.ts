...
const descriptors = Object.fromEntries(
  ["indexedDB", "createImageBitmap", "localStorage"].map((key) => [
    key,
...
test("PNG/JPEG/WebP dan batas 5 MB; menolak kosong, terlalu besar dan MIME lain", () => {
  for (const type of ["image/png", "image/jpeg", "image/webp"])
    expect(validateEvidenceFile({ type, size: EVIDENCE_MAX_BYTES, name: "bukti.jpg" })).toBeNull();
  for (const value of [
    { type: "image/svg+xml", size: 100, name: "bukti.svg" },
    { type: "image/png", size: 0, name: "bukti.png" },
    { type: "image/png", size: EVIDENCE_MAX_BYTES + 1, name: "bukti.png" },
  ])
    expect(validateEvidenceFile(value)).not.toBeNull();
...
test("upload → draft → kirim → validasi menjaga blob/id dan tidak publik", async () => {
  const uploaded = await mockRepository.uploadReportEvidence(
    manager,
...
test("lampiran hilang/nama salah/lintas scope tidak membuat laporan", async () => {
  ...
  for (const override of [
    ...
    expect((await mockRepository.submitLaporCepat(manager, { ...input, ...override })).ok).toBe(
      false,
    );
...
test("gambar rusak/dekode gagal atau role dilarang tidak menyimpan aset", async () => {
  expect(
    (
      await mockRepository.uploadReportEvidence(
        { id: "USR-001", name: "Penguji" },
        input.institutionCode,
        file(),
      )
    ).ok,
  ...
  expect(
    (await mockRepository.uploadReportEvidence(manager, input.institutionCode, file())).ok,
  ).toBe(false);
...
test("reset demo membersihkan blob bukti dan referensi laporan", async () => {
  ...
  if (!uploaded.ok || !uploaded.id) throw Error("Upload gagal");
  await mockRepository.submitLaporCepat(manager, {
    ...input,
    evidenceAssetId: uploaded.id,
    evidenceName: "bukti.png",
  });
