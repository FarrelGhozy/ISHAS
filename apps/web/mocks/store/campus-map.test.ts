...
test("multi-jawaban memakai lineage dan titik sumber masing-masing", () => {
  const version = getState().instrumentVersions.find(
    (item) => item.id === getState().activeInstrumentVersionId,
  )!;
  ...
  expect(
    findings.find((item) => item.sourceAnswerId === "IND-K3L-001")?.locationSnapshot?.point,
  ).toEqual({ x: 20, y: 20 });
  expect(
