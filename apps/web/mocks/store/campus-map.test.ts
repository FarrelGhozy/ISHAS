...
test("penggantian memerlukan pengelola scope, persetujuan dan versi aktif yang belum berubah", () => {
  ...
  expect(selectPublicCampusMap(getState(), "PSN-0018").items.filter((item) => item.versionId).every((item) => item.versionId === "CAMPUS-PSN-0018-v1")).toBe(true);
...
test("multi-jawaban memakai lineage dan titik sumber masing-masing", () => {
  const version = getState().instrumentVersions.find((item) => item.id === getState().activeInstrumentVersionId)!;
  ...
  storeActions.saveSelfAssessmentDraft({ id: "SELF-uji", institutionCode: "PSN-0018", reporterName: "Penguji", instrumentVersionId: version.id, answers, activeIndex: 0, updatedAt: new Date().toISOString() });
...
test("jawaban sesuai tidak menghasilkan pin temuan generik", () => {
  ...
  storeActions.saveSelfAssessmentDraft({ id: "SELF-sesuai", institutionCode: "PSN-0018", reporterName: "Penguji", instrumentVersionId: version.id, answers, activeIndex: 0, updatedAt: new Date().toISOString() });
