...
export function migrateV4(value: unknown): unknown {
  if (!value || typeof value !== "object" || (value as IshasState).schemaVersion !== 4)
    return value;
  ...
  migrated.campusPlans = structuredClone(SEED.campusPlans).filter((plan) =>
    migrated.institutions?.some((institution) => institution.code === plan.institutionCode),
  );
  migrated.institutions?.forEach((institution) => {
    institution.activeCampusPlanVersionId = migrated.campusPlans.find(
      (plan) => plan.institutionCode === institution.code,
    )?.id;
  });
  migrated.reports?.forEach((report) => {
    delete report.locationSnapshot;
    report.planPoint = null;
  });
  migrated.findings?.forEach((finding) => {
    delete finding.locationSnapshot;
  });
  migrated.selfAssessmentSnapshots?.forEach((snapshot) => {
    Object.values(snapshot.answers ?? {}).forEach((answer) => {
      delete answer.locationSnapshot;
      answer.planPoint = null;
    });
  });
  Object.values(migrated.selfAssessmentDrafts ?? {}).forEach((draft) => {
    Object.values(draft.answers ?? {}).forEach((answer) => {
      delete answer.locationSnapshot;
      answer.planPoint = null;
    });
  });
...
// D-16: v6 → v7 mempertahankan seluruh record/ID; hanya menambah
// `instrumentDocs` (pustaka PDF per indikator, independen dari versioning).
export function migrateV6(value: unknown): unknown {
  if (!value || typeof value !== "object" || (value as IshasState).schemaVersion !== 6)
    return value;
  const migrated = structuredClone(value) as IshasState;
  migrated.schemaVersion = 7;
  if (!Array.isArray(migrated.instrumentDocs))
    migrated.instrumentDocs = structuredClone(SEED.instrumentDocs ?? []);
  return migrated;
}
  ...
  return (
    ...
    state.instrumentVersions.every(
      (v) =>
        ...
        v.dimensions.every((d) => d && typeof d === "object" && Array.isArray(d.indicators)),
    ) &&
    state.selfAssessmentSnapshots.every((s) => s && s.answers && typeof s.answers === "object") &&
    !!state.selfAssessmentDrafts &&
    typeof state.selfAssessmentDrafts === "object" &&
    !!state.indexHistory &&
    typeof state.indexHistory === "object" &&
    Object.values(state.indexHistory).every(Array.isArray) &&
    !!state.counters &&
    Number.isSafeInteger(state.counters.report) &&
    state.counters.report > 0
  );
}
...
export function saveState(state: IshasState): void {
  ...
  try {
    ...
    throw new Error(
      "Data tidak dapat disimpan. Periksa ruang dan izin penyimpanan browser, lalu coba lagi.",
    );
  }
}
