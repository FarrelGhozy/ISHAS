...
export function migrateV6(value: unknown): unknown {
  if (!value || typeof value !== "object" || (value as IshasState).schemaVersion !== 6) return value;
  const migrated = structuredClone(value) as IshasState;
  migrated.schemaVersion = 7;
  if (!Array.isArray(migrated.instrumentDocs)) migrated.instrumentDocs = structuredClone(SEED.instrumentDocs ?? []);
  return migrated;
}
...
function isValidState(value: unknown): value is IshasState {
  ...
  const arrays = [state.campusPlans, state.institutions, state.users, state.reports, state.selfAssessmentSnapshots,
    ...
    state.instrumentVersions, state.instrumentDocs, state.auditEvents, state.notifications];
  return state.schemaVersion === MOCK_SCHEMA_VERSION && arrays.every(Array.isArray)
    ...
    && state.instrumentVersions.every((v) => v && typeof v === "object" && Array.isArray(v.dimensions) && v.dimensions.every((d) => d && typeof d === "object" && Array.isArray(d.indicators)))
...
export function loadState(): IshasState {
  ...
  try {
    const raw = localStorage.getItem(MOCK_STORAGE_KEY) ?? localStorage.getItem("ishas-mock-v6") ?? localStorage.getItem("ishas-mock-v5") ?? localStorage.getItem("ishas-mock-v4");
    if (raw) {
