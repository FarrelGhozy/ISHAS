...
export function Page() {
  ...
  const types = [...new Set(state.auditEvents.map((x) => x.objectType))];
  const events = useMemo(
    () =>
      state.auditEvents.filter(
        (x) =>
          (type === "Semua" || x.objectType === type) &&
          `${x.action} ${x.actorName} ${x.objectId} ${x.note ?? ""}`
            .toLowerCase()
            .includes(query.toLowerCase()),
      ),
    [state.auditEvents, query, type],
  );
  return (
    <section className="flex flex-col gap-4">
      <header>
        ...
        <p className="text-sm text-secondary-text">
          Jejak perubahan data sistem yang tersimpan secara kronologis.
        </p>
      </header>
      <div className="flex gap-2">
        <input
          aria-label="Cari audit"
          className="min-h-11 flex-1 rounded border border-line-soft px-3"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Cari aksi, pelaku, objek, atau catatan…"
        />
        <select
      ...
      {events.length ? (
          ...
          {events.map((x) => (
            <article key={x.id} className="p-4 text-sm">
              ...
              <p className="text-secondary-text">
                {x.actorName} · {x.objectId} · {new Date(x.at).toLocaleString("id-ID")}
              </p>
        ...
        <EmptyState
          ...
          description="Ubah pencarian atau filter objek."
        />
      )}
    </section>
  );
