...
import { hitungIndexSummary, knownPeriods, resolvePeriodeParam } from "~/mocks/processors/dashboard-aggregate";
...
function Validator({ state, id, name }: { state: ReturnType<typeof useMockState>; id?: string; name?: string }) { const user = selectUserById(state, id); const display = name ?? user?.name; return display ? <p className="mt-3 flex items-center gap-1 text-sm text-secondary-text"><MapPin size={14} />Divalidasi oleh {display}</p> : null; }
