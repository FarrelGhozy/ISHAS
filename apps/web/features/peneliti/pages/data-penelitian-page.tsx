...
export function Page() {
  ...
  const [institution, setInstitution] = useState("Semua");
  const rows = useMemo(
    () =>
      state.reports.filter(
        (x) =>
          x.channel === "penilaian-mandiri" &&
          (institution === "Semua" || x.institutionCode === institution) &&
          `${x.id} ${x.reporterName} ${x.title}`.toLowerCase().includes(query.toLowerCase()),
      ),
    [state.reports, institution, query],
  );
    ...
    <section className="flex flex-col gap-4">
      <header>
        ...
        <p className="text-sm text-secondary-text">
          Katalog snapshot penilaian mandiri beserta provenance dan status validasinya.
        </p>
      </header>
      ...
      {rows.length ? (
        <div className="surface divide-y divide-line">
          {rows.map((x) => {
            ...
            const inst = state.institutions.find((i) => i.code === x.institutionCode);
            return (
              <article className="flex flex-wrap items-center gap-3 p-4 text-sm" key={x.id}>
                <div className="mr-auto">
                  ...
                  <p className="text-xs text-faint">
                    {snapshot?.instrumentVersionId ?? "Versi tidak tersedia"} ·{" "}
                    {snapshot ? Object.keys(snapshot.answers).length : 0} jawaban
                  </p>
                </div>
