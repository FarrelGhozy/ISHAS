    ...
    <section className="flex flex-col gap-4">
      <header>
        ...
        <p className="text-sm text-secondary-text">
          Tinjau struktur dimensi, indikator, dan aturan pengumpulan bukti. Versi Published
          terkunci; perubahan hanya pada Draft (FLOWS §7).
        </p>
      </header>
        ...
        <>
          ...
          {isDraft ? (
            ...
            <p className="rounded-lg border border-line bg-strip p-3 text-sm text-secondary-text">
              Versi {version.status} terkunci. Buat draft baru di Versioning untuk mengubah
              struktur.
            </p>
          )}
          {dimensions.map((d) => {
              ...
              <article className="surface overflow-hidden" key={d.id}>
                <div className="border-b border-line bg-strip px-4 py-3">
                  ...
                  <p className="text-xs text-faint">
                    {d.id}
                    {d.categoryId ? ` · ${d.categoryId}` : ""} · {indicators.length} indikator
                  ...
                  <div className="grid gap-2 border-t border-line bg-strip p-4 md:grid-cols-3">
                    ...
                    <label className="text-xs font-bold">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          patchIndForm(d.id, {
                            answerType: e.target.value as IndForm["answerType"],
                          })
                        }
                      >
                    ...
                    <div className="flex flex-wrap gap-3 text-xs md:col-span-2">
                      ...
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          onChange={(e) =>
                            patchIndForm(d.id, { evidenceRequired: e.target.checked })
                          }
                        />
                        Bukti wajib
                      </label>
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          onChange={(e) =>
                            patchIndForm(d.id, { locationRequired: e.target.checked })
                          }
                        />
                        Lokasi wajib
                      </label>
