...
export function Page() {
  ...
  const assessments = state.reports.filter((x) => x.channel === "penilaian-mandiri");

  return (
    <section className="flex flex-col gap-4">
      <header>
        <p className="kicker">Kualitas data</p>
        <h1 className="text-2xl font-extrabold text-heading">Validasi & publikasi</h1>
        <p className="text-sm text-secondary-text">
          Pantau kelengkapan snapshot dan kelayakannya sebagai sumber hasil publik.
        </p>
      </header>
        ...
        {assessments.map((x) => {
          ...
          return (
            <article className="flex flex-wrap items-center gap-3 p-4" key={x.id}>
              ...
              <span className={`status ${complete ? "status-green" : "status-red"}`}>
