  ...
  const run = (action: () => { ok: boolean; error?: string }) => {
    ...
    setMessage(
      r.ok
        ...
        : (r.error ?? "Perubahan gagal disimpan."),
    );
  };
  return (
    <section className="flex flex-col gap-5">
      <header>
        <p className="kicker">Master lokasi</p>
        <h1 className="text-2xl font-extrabold text-heading">Lokasi dan denah</h1>
        <p className="mt-1 text-sm text-secondary-text">
          Area baru langsung tersedia pada formulir lapor pesantren ini.
        </p>
      </header>
      {message && (
      ...
      <CampusPlanManager key={user.institutionCodes[0]} user={user} />
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="surface p-4">
          <h2 className="font-bold">Tambah gedung</h2>
        ...
        <section className="surface p-4">
          <h2 className="font-bold">Tambah area</h2>
          <label className="mt-3 block text-sm font-bold">
            Gedung
            <select
              className={field}
      ...
      <section className="surface p-4">
        <h2 className="font-bold">Gedung dan keterangan lantai</h2>
        <div className="mt-3 space-y-3">
          {buildings.map((building) => (
            <article key={building.id} className="rounded border border-line-soft p-3">
              <h3 className="font-bold">
                {building.code} · {building.name}
              </h3>
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                {building.floors.map((item) => (
                  <div key={item.id} className="rounded bg-surface-muted p-3 text-sm">
                    <strong>{item.name}</strong>
                    <p className="mt-1 text-secondary-text">
                      {item.planVersion
                        ? `${item.planVersion} · ${item.planFile}`
                        : "Belum ada denah"}
                    </p>
                    <p className="mt-2 text-xs text-secondary-text">
                      Denah per lantai lama hanya catatan historis. Pelaporan memakai Denah
                      Pesantren di atas.
                    </p>
                    {item.planHistory?.length ? (
                      <p className="mt-2 text-xs text-secondary-text">
                        Riwayat: {item.planHistory.map((x) => x.version).join(", ")}
                      </p>
                    ) : null}
                  </div>
                ))}
      ...
      <section className="surface p-4">
        <h2 className="font-bold">Daftar area</h2>
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {areas.map((x) => (
            <div key={x.id} className="rounded border border-line-soft p-3 text-sm">
              <strong>{x.name}</strong>
              <p className="text-secondary-text">
                {buildings.find((b) => b.id === x.buildingId)?.name} · {x.floor} · {x.zone}
              </p>
            </div>
          ))}
        </div>
