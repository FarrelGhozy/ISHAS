  ...
  const progress = useMemo(
    () =>
      recommendations.length
        ? Math.round(recommendations.reduce((n, x) => n + x.progress, 0) / recommendations.length)
        : 0,
    [recommendations],
  );
  const dimensions =
    ...
    <section className="flex flex-col gap-4">
      <header>
        ...
        <p className="mt-1 text-sm text-secondary-text">
          ...
          dibuat {new Date().toLocaleDateString("id-ID")} oleh {user.name}
        </p>
      </header>
      ...
      <section className="surface p-4">
        <h2 className="font-bold">Dimensi instrumen</h2>
